...
def chat(request: ChatRequest) -> ApiResponse:
    logging.info(f"与AI模型交互！{request.session_id}: {request.message}")

    # 1. 加载json中的会话信息
    session_path = get_session_file_name(request.session_id)
    with open(session_path, "r", encoding="utf-8") as f:
        session_data = json.load(f)
        logging.info(f"加载会话信息：{session_data}")
    ...
    logging.info(f"构建AI大模型交互的内容：{messages}")
    ...
    response = client.chat.completions.create(
            ...
            temperature=1.5 # 调整温度以控制生成的随机性
        )
    # 4. 获取响应内容
    ai_response = response.choices[0].message.content
    logging.info(f"AI模型的回复：{ai_response}")

    # 5. 更新会话信息
    messages.pop(0) # 移除系统提示词
    messages.append({"role": "assistant", "content": ai_response})
    session_data["messages"] = messages
    logging.info(f"更新会话信息：{session_data}")

    # 6. 保存会话信息到json文件
    with open(session_path, "w", encoding="utf-8") as f:
        json.dump(session_data, f, ensure_ascii=False, indent=4)
        logging.info(f"保存会话信息到json文件：{session_path}")
...
def get_sessions() -> ApiResponse:
    ...
    logging.info(f"获取会话文件：{session_files}")

    # 2. 获取文件名中的会话标识
    session_ids = [f.split(".")[0] for f in session_files if f.endswith(".json")]
    session_ids.sort(reverse=True) # 按时间倒序排序
    logging.info(f"获取会话标识：{session_ids}")

    # 3. 返回数据
    return ApiResponse(code=200, message="会话信息获取成功", data=session_ids)
...
def get_session(session_id: str) -> ApiResponse:
    ...
    logging.info(f"获取会话文件：{session_file}")

    # 2.读取会话信息
    with open(session_file, "r", encoding="utf-8") as f:
        session_data = json.load(f)
        logging.info(f"读取会话信息：{session_data}")
...
def handle_exception(request: Request, exc: Exception):
    logging.error(f"路径：{request.url} 发生异常：{exc}")
    return JSONResponse(content={"code": 500, "message": "服务器内部错误", "data": None}, status_code=500)


# 运行应用
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000) # 关闭日志：access_log=False